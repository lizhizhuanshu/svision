...
static int notColor(lua_State*L);
...
{
	luaL_checkversion(L);
	if(luaL_newClassMetatable(NotColor,L)){
		static luaL_Reg methods[] =
			{
				{"__gc",lua::finish<NotColor>},
				{nullptr,nullptr}
			};
    ...
    lua_pop(L,1);
		...
		if(luaL_newClassMetatable(StringNotColor,L)){
		static luaL_Reg methods[] =
			{
				{"__gc",lua::finish<StringNotColor>},
				{nullptr,nullptr}
			};
		luaL_setfuncs(L, methods, 0);
        lua_pushvalue(L, -1);
	...
	if(luaL_newClassMetatable(PngImage,L))
	{
        static luaL_Reg methods[] =
			{
				{"load",pngLoad},
...
void injectMethodTo(lua_State*L,int tableIndex)
{
	static luaL_Reg methods[] =
	{
		{"getColor",getColor},
...
static int notColor(lua_State*L){
	if (lua_isinteger(L, 1)) {
		...
		luaL_pushNewObject(NotColor,L,color);
	}else if (lua_isstring(L, 1)) {
		size_t size;
		const char* str = lua_tolstring(L, 1, &size);
		luaL_pushNewObject(StringNotColor,L,str,size);
	}else {
		luaL_error(L,"arg %d , export integer or string , now %s",1,luaL_typename(L,1));
	}
	return 1;
}
...
static int pngGetSize(lua_State*L)
{
	...
	if(png->origin_)
	{
		lua_pushinteger(L,png->width_);
		lua_pushinteger(L,png->height_);
	}else{
		lua_pushinteger(L,-1);
		lua_pushinteger(L,-1);
	}
...
int getColorCount(lua_State* L)
{
	...
	}else if(lua_isstring(L,6))
	{
		...
		count = getColorCount(bitmap, x, y, x1, y1,&color, shift);
	}else if (luaL_testObject(NotColor,L,6)) {
		NotColor* color = lua::toObject<NotColor>(L, 6);
		count = getColorCount(bitmap, x, y, x1, y1,color, shift);
	}else if (luaL_testObject(StringNotColor, L, 6)) {
		StringNotColor *color = lua::toObject<StringNotColor>(L, 6);
		count = getColorCount(bitmap, x, y, x1, y1,color, shift);
	}
	else
	{
